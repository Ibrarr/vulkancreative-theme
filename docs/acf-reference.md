...
- **Partner Logos** (PHP-registered in `inc/trust-signals-fields.php`, group `group_vc_partner_logos`, Global Settings, Aug 2026): `partner_logos_heading` (text, the strip label) + `partner_logos` repeater: `logo` (image, required; must read on dark), `logo_light` (image, optional light-surface artwork), `name` (text, alt fallback), `url` (optional link), `dark_only` (true_false: white artwork hidden on light surfaces until a light file is added). Rendered by `template-parts/partner-logos.php`.
