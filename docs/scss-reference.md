...
## Case study pages (July 2026)

`assets/css/components/case-study/` — scoped to the auto body classes `.post-type-archive-case_study` and `.single-case_study`: `_case-study.scss` (uppercase h2 + red span rule, PHP-assigned surface pairs with chained `body.dark-mode` overrides, `html.js` pre-hide + `vc-cs-reveal-failsafe`) plus `components/{_index,_filter,_hero,_overview,_narrative,_results,_testimonial,_related,_cta}.scss`. Recipes are mirrored from the work family (the `.project-cta` mirroring precedent) so /work/ ships untouched; the filter chips restate the work recipe under the CS scope with the same class names (the unscoped `@at-root` dropdown/dark rules in the project partial already cover both). The shared card lives in `common/_case-study-card.scss` (archive grid + service strip). The results band restates the service results-anchor recipe as `.cs-results` (markup keeps `.results`/`.stat-number` for counter.js). The static testimonial rides `common/_testimonial-spotlight.scss` via `&.spotlight-static blockquote` selector extensions (`--no-photo` drops the portrait column). The project single gained `components/project/components/_case-study-band.scss` inside the existing project scope. New pre-hidden selectors live in `misc/_motion.scss`. Full notes in `docs/case-studies-system.md`.
