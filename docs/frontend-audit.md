...
- **EM-04** · email · important · `Status: open` — **No submitter autoresponder on any form.** Someone who enquires gets no email at all. Proposal pack (per-form subject + body in the house voice) drafted at fix time; applying it is DB work under the agreed approval.
- **EM-05** · email · minor · `Status: open` — Subject drift: forms 2 and 3 say "New submission from", forms 5 to 10 "New enquiry from". Align.
- **EM-06** · copy · minor · `Status: open` — **Form 2's confirmation is off-voice**: "We're excited to build something powerful together... discuss your vision" against form 5's plain one-working-day promise. Rewrite to match the house voice and promise.
...
- **META-01** · seo-schema · important · `Status: open` — **Missing meta descriptions:** `/contact/` (existing CON-01), `/seo-ai-search/`, both legal pages, all 19 category term pages (the term description doubles as the hero standfirst, so writing them pays twice). og:description absent on the same set.
...
- **SEO-01** · seo-schema · minor · `Status: open` — **`/llms.txt` is stale and part broken:** generated by Yoast v27.3 (site runs v28.2), backslash-escaped markdown throughout, one typo ("Fee Business Website"), old post titles, and thin coverage (5 pages, 5 of 15 posts, no services). Regenerate once the services noindex decision lands.
