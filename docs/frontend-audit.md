...
- **SEO-01.** The remaining `[SAMPLE]` stat rows were cleared from the service terms (the section hides when empty), the `service` taxonomy is now **indexable**, and the 18 child terms carry per-term noindex so only the six pillars enter the index. Yoast's term indexables were purged so the cache rebuilds. Verified: pillar `index, follow` with canonical and no visible [SAMPLE]; child `noindex, follow`.
