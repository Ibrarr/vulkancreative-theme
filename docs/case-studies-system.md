...
- **Where the links surface:**
  ...
  - Case study single: the fact ledger's Project row, the borrowed "What we delivered" list and the `.cs-related` feature panel (the hero carries no project button; it was removed to keep the hero lean).
...
5. **Testimonial**: the shared spotlight in its static variant: `.testimonial-spotlight.spotlight-static` (plus `no-photos` on the root when the testimonial has no portrait, in which case the photo panel is not printed at all: no stand-in portraits), fed by the `cs_testimonial` post object into the `testimonial` CPT. No Splide anywhere on these pages. The static styles ride the carousel partial via `&.spotlight-static blockquote` selector extensions in `common/_testimonial-spotlight.scss`.
...
- **Real content (Sep 2026):** the seeder (`inc/case-study-seed.php`) and the three `[SAMPLE]` case studies are gone. Four real case studies went live locally on 17 Sep 2026, each paired to its work entry through `pj_case_study`: The Boundary SW6, Source Pay, Minds Behind the Bite and Bite Talent, all built on figures from the clients' own monthly reports, with the period and measurement basis stated in each Results section. Featured on the homepage: The Boundary, Source Pay, Bite Talent. Leigh Blanks, Now Sold and The Broker House are drafted in the content repo under `held/case-studies/` and wait on real figures from Ibrar; nothing is ever invented for a named client.
- **Case study images are wall boards, not the work boards.** `cs_image` is shown at 21:9 (single hero), 16:10 (cards), near square (homepage desktop stage) and portrait with text over the lower half (homepage mobile card), so the key content sits in the intersection of those crops: on the 2400 x 1500 dark ground, five columns by two rows of 400 x 500 tiles (4:5, the posts' own ratio, so whole posts show and the block fills the safe height) with 24px gaps, the centre three columns (x 576 to 1824, y 238 to 1262, inside the shared safe rectangle x 555 to 1845, y 236 to 1264) holding the client's white logo tile plus five real posts, the outer columns further posts at 42% opacity. Approach gallery images are rows of whole tiles inside x 200 to 2200, y 236 to 1264. Build them from HTML, render headless and assert the geometry per board.
- **Yoast noindex** (`noindex-case_study` + `noindex-ptarchive-case_study` in `wpseo_titles`) is off locally; the `[SAMPLE]` case studies were deleted on 16 Sep 2026 and real ones publish through the content pipeline with a `date` key that orders the archive. Confirm the toggles on live at deploy.
