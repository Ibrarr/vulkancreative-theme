...
- **Conditional-logic remap.** The same pass rewrites any field conditional-logic rule still keyed on a retired pillar slug to the live slug, so the step-2 "current website" / "new or redesign" fields keep firing. The map is `vc_service_slug_aliases()` in `inc/template-functions.php` (shared with the URL shim and the archive chips; `digital-marketing`→`strategy-analytics`, `paid-search-ppc`→`paid-media`, `content-creation` and `content-marketing`→`content-social` since the Aug 2026 Content & Social rename). Add to that one map if a pillar is renamed again.
