...
- **Steps (GF pagination type `steps`):** Services · Project · About You · Contact.
  1. **Services** — checkbox field 1 (`service-picker` cssClass), choices are the six service taxonomy slugs + `something-else`, multi-select, required. Hidden UTM fields 13–16 (source/campaign/term/content, prepopulated) sit on this page.
  2. **Project** — textarea 2 (required); website field 3 (optional, shown if any of `web-design-development` / `seo-ai-search` / `digital-marketing` / `paid-search-ppc` is ticked); radio 4 new-build-vs-redesign (optional, web only); radio 5 timeframe (optional).
...
- **Notification:** to `info@vulkancreative.test` (mirrors form 2; point at the live inbox at deploy), subject "New enquiry from {form_title}", body `{all_fields}`. **Confirmation:** inline message with the one-working-day promise.
...
- `inc/gravity-forms.php` (required from `functions.php`) — the theme's first GF hooks: `gform_confirmation_anchor` → false for `enquiry-form` forms (theme JS owns scrolling), and `gform_field_choice_markup_pre_render` → injects the service SVG watermarks (`.choice-card-icon`) into the picker labels, keyed on choice value.
...
Field labels, descriptions, choices, button text, validation messages, the About You optional-step note (HTML field 18), the confirmation and the notification all live in GF admin → form 5. The "It takes about a minute." line is the `ct_form_note` ACF field on the Contact page.
