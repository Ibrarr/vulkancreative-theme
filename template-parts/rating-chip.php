...
// line, and the wording follows Google's brand rules ("on Google", never
// "Google rating"; stars kept on their own row away from the G; an as-of date
// beside the overall figure). Without count or link it falls back to the
// neutral unlinked presentation.
...
?>
<?php if ( $google_ok ) :
	$reviews_word = 1 === $gr['count'] ? 'review' : 'reviews';
	$chip_label   = $gr['rating'] . ' from ' . $gr['count'] . ' ' . $reviews_word . ' on Google' . ( $gr['as_of'] ? ', as of ' . $gr['as_of'] : '' ) . '. Opens our Google profile in a new tab.';
	?>
	<a class="rating-chip" href="<?php echo esc_url( $gr['url'] ); ?>" target="_blank" rel="noopener" aria-label="<?php echo esc_attr( $chip_label ); ?>">
		...
		<span class="rating-line" aria-hidden="true">
			...
			<span class="rating-text"><span class="rating-value"><?php echo esc_html( $gr['rating'] ); ?></span> from <?php echo (int) $gr['count']; ?> <?php echo esc_html( $reviews_word ); ?> on Google</span>
		</span>
		<?php if ( $gr['as_of'] ) : ?>
