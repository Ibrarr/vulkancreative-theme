...
- **Copy:** sentence-case headings that say what the section is; no "that performs" family; "in-house" appears once per service page, in the intro's benefit sentence, never as a suffix or a FAQ; titles never clamp; testimonials never show a stand-in portrait.
...
- **Database side:** `replay-prelaunch-audit.php` (outside the theme, dry run by default, `--apply`, `--only=`), then regenerate Yoast's llms.txt on the target site.
