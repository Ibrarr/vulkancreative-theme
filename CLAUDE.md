...
- **About press section (the print run, Ibrar's second review, Aug 2026):** `.about-press` after `.about-proof` (#F5F5F5 light / #121212 dark, continuing the alternation), `ab_press_*` fields in about-us.json: a split head (three-part heading left; `ab_press_body` + ghost CTA `ab_press_link_label`/`ab_press_link_url` right, new tab; no wordmark or credit line, both rejected), then the **print run** across the band's base: `ab_press_image` (the feature spread, seeded as a 1708px WebP) repeated as overlapping, fanned copies (`.press-print`, each copy overlapping the one before by `width - step`, so every visible slice is the headline and the statue) on two slow counter-scrolling CSS-keyframe rows (`.press-rack-row.is-front` 470/300px copies, `.is-back` smaller and 35% opacity behind), tilted as one `.press-rack-plane` (`rotate(-5deg)`, origin bottom-right so the right end sits on the seam) inside a fixed-height `.press-rack` stage (440/300px) whose `mask-image` sinks the top into the band and clips the copies at the seam; hairline paper edges, 2px corners, `grayscale(0.25)` rest, no shadows; even copy counts per set keep the stacked offsets seamless across the loop. The band uses `overflow: clip`, never `hidden` (hidden makes it a scroll container that anchors and focus scroll internally by the run's overflow), and `padding-bottom: 0` at every width (the section mixin's small-screen padding must be re-zeroed). The rack is `aria-hidden` (decorative; the head carries the information, copies carry `alt=""`). Reveals: `.press-body`, `.press-cta` and `.press-rack` fade (the tilt lives on the inner plane, never on the fade target); the marquee is the homepage text-marquee recipe and the reduced-motion safety net (plus an explicit `animation: none`) leaves a still rack. The section hides when every raw press field is blank; no image hides the run. The template tolerates a bare attachment ID from the image field (`acf_get_attachment()` fallback), after a programmatic group import once rewrote the field as `text` and fatalled the page.
