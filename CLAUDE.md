...
- `docs/prelaunch-audit.md`: the Sep 2026 pre-launch pass: script loading, first-paint and motion rules, the shared testimonial partial, the hub directory, the SEO plumbing and the replay pack.
...
- **Branches and deploy flow (recorded Aug 2026):** the theme folder is the git repo (`origin` = `github.com/Ibrarr/vulkancreative-theme`). `main` is production and only moves by a reviewed release; `staging` is the integration branch that the staging site runs, and feature branches are cut from it (`feat/`, `fix/`, `docs/` prefixes) and land on it with a `--no-ff` merge commit once Ibrar says push. The release is a draft PR `staging` to `main` (PR #2 opened 21 Aug 2026 carries audit pass 2, Content & Social and the trust signals), merged only after Ibrar signs staging off. The theme carries no data: every feature's database side ships as a replay pack (Content & Social: `migrate-content-social.php`, see docs/services-system.md; trust signals: `replay-trust-signals.php`, dry run by default, `--apply` to write, handed over 21 Aug) that runs on the target database after the theme deploys, Content & Social first. How the host pulls `origin/staging` (automatic or a manual pull) is Ibrar's to confirm and should be noted here when known.
- **Launch deploy (Ibrar, 17 Sep 2026):** for the launch he copies the whole local site, files and database, to staging and then production and search-replaces the URL. The local database is therefore the source of truth: no seeders, blocks or replay packs are needed for it, and copy edits go straight into the local database (after asking). After the URL change on each environment: regenerate `llms.txt`, flush permalinks, purge LiteSpeed. The parked go-live checks are listed in `docs/prelaunch-audit.md` under "Before go-live".
...
- **Run browser QA headless from a script, not through the Playwright MCP (Ibrar's instruction, Sep 2026: "stop asking to approve playwright action, just do it").** The MCP raises a permission prompt per action. A Node script run through Bash (`require` the npx-cached `playwright`, `chromium.launch({ headless: true, channel: 'chrome' })`, `ignoreHTTPSErrors`) needs none, covers the 375/768/1024/1440 by two modes matrix in one go, and can measure as well as capture. Keep the scripts and captures in the session scratchpad.
...
**Typical module** (`hero.js`): new sections use the IntersectionObserver reveal with the reduced-motion guard, so content can never be left hidden (ScrollTrigger/SplitText remain available for scrubbed or split-text work; full pattern catalogue in `docs/js-reference.md`). Since Sep 2026 this pattern is for **lists of objects only** (cards, steps, rows staggering in): body copy, forms and FAQs are never hidden, a hero's text is never hidden below lg (it is the LCP element), and a plain fade-up on every block is banned; see Motion and interaction below.
...
if ( is_page_template( 'page-templates/page-your-page-name.php' ) ) {
    vc_enqueue_bundle( 'your-page-name' ); // deferred, in the head, no jQuery dependency
}
```
...
- Section reveals use IntersectionObserver + GSAP with nothing pre-hidden without JS or under reduced motion (the `reveal.js` pattern); add new section h2s to `reveal.js`'s selector lists for the standard SplitText line reveal (always `aria: 'none'` plus `revert()` on completion: the default writes `aria-label` onto the element, which is prohibited on a `<p>`).
- **No blanket fade-ups (Ibrar's ruling, Sep 2026).** The heading line reveals stay; the generic 24px fade on every block went. Motion goes to lists of objects, chosen per section: cards and steps stagger 70 to 80ms apart, the About founder panels clip in, partner badges cascade, the rule above each stat draws with its count-up, the hub directory rows rise one by one. Body copy, sub-headings, sidebars, forms, FAQs and article bodies simply sit there.
- **The LCP element never waits for script.** Below lg every hero paints with the first frame through the CSS entrance in `misc/_motion.scss` (it starts at `opacity: 0.01`, because Chrome ignores opacity 0 for LCP); JS hero sequences run at lg and up only (`heroOnJs` in each reveal module). The homepage poster is server-rendered in a `<picture>` with a preload.
- Hover glows are heat, not drop shadows: `$vc-glow-card`, `$vc-glow-card-dark` and `$vc-glow-button` carry no y offset. Offset shadows are for floating layers only (scrolled header, mega menu, filter dropdown).
...
- **Arrows never nudge on hover (Ibrar, Sep 2026).** The 3 to 6px arrow slide came off every link, card and button sitewide; hover feedback on an arrow is colour only. The check is a script that hovers every link and button and diffs computed transforms (only an image zooming in its frame, the service card watermark drift and the dropdown caret flip may change).
...
- **One CTA label per job:** "Start a Project" for the primary CTA everywhere (header included), "Send Enquiry" on general forms, and each offer page repeats its own single label ("Get Your Free Website", "Get My Free Report", "Book a Free Consultation").
- **Copy:** sentence-case headings that say what the section is; no "that performs" family; "in-house" appears once per service page, in the intro's benefit sentence, never as a suffix or a FAQ; titles never clamp; testimonials never show a stand-in portrait. The site calls itself a "London digital marketing agency" (not "digital agency"), and names social media management wherever it lists services in metadata, because marketing and social are the larger share of the work. Say the founders run it once per page at most (homepage: the hero, plus the one Why proof line); a section that wants to repeat it should say something new instead. Cost FAQs carry no figures, by Ibrar's choice: they name what drives the price and how it is agreed.
- **Shared parts:** `template-parts/testimonial-spotlight.php`, `template-parts/offer-links.php`, `vc_image()` for every image, `vc_enqueue_bundle()` for every script.
- **Database side:** the launch copies the local database (see Launch deploy above), so `replay-prelaunch-audit.php` is a record, not a deploy step. `llms.txt` is a static file with absolute URLs: regenerate it on each environment (`define("DOING_CRON", true)` then `do_action("wpseo_llms_txt_population")`; the theme strips Yoast's byte order mark, which Lighthouse rejects).
- **Review round (17 Sep 2026):** Splide lists and slides are `<div>`s (Splide's `role="group"` is invalid on `<li>`), which takes Lighthouse's Agentic Browsing to 3/3; process steps fill their own top rule at lg and up (`.step-fill`), never the gap between steps; `vc_service_icon_url()` stops children borrowing their pillar's icon (new Slab icons must be supplied as SVGs: the Font Awesome token on file has no Pro SVG scope); author photos take the About duotone; the About film is a self-hosted 1080p encode.
...
**Restructured to pillars + children (July 2026).** The flat six terms are now 6 parent pillars, each with child services, at `/services/{pillar}/{child}/`. `taxonomy-service.php` forks on `$term->parent` (pillar = the full single-service layout plus the children grid; leaf = the single-service layout), and an embedded Gravity Form 2 now sits on **every** service page. The per-term `order` field was replaced by a global **Service list** on Global Settings (order + per-location visibility), read via `vc_ordered_services($location)` / `vc_service_children()`; the header renders a **dynamic mega menu** from the taxonomy (`inc/mega-menu.php`, `header/components/_mega.scss`); and the three seeders were deleted. Full detail in the "Restructure to pillars + children" section of `docs/services-system.md`.

**Content & Social + Social Media Management (August 2026).** The Content Marketing pillar is now **Content & Social** (slug `content-social`; the six pillars read Web Design & Development, SEO & AI Search, Paid Media, Content & Social, Branding, Strategy & Analytics) with a third child, **Social Media Management** (`/services/content-social/social-media-management/`), the organic social offer. With it: child order is **editable** (each Service List row has a `children` sub-repeater; `vc_service_children()` returns listed children first, unlisted ones after them alphabetically, so nothing ever disappears); an editorial `sv_related_services` picker on the `sv_` group overrides the derived related strip (used to cross-link Social Media Management and Paid Social); one alias map `vc_service_slug_aliases()` feeds the URL shim (which now also 301s stale `/services/{old-pillar}/{child}/` URLs), the enquiry-form conditional-logic remap and the `?service=` chips; the three legacy top-level terms (Content Creation, Digital Marketing, Paid Search & PPC) were retagged onto live pillars and deleted; **every service page is indexable** (the 18 children's per-term noindex is off); and blog category "Content Marketing" is now "Content & Social" (`/blog/category/content-social/`, 301 from the old URL). Full detail, and the deploy-day replay pack for the DB side, in the "Content & Social + Social Media Management" section at the top of `docs/services-system.md`.
...
**Results band sizing and label rules (Ibrar's review, Sep 2026).** The case study results band (`single-case_study.php` + `case-study/components/_results.scss`) sizes its figures from the set itself: the template prints the longest value's length as `--stat-chars`, each `.stat` is a size container, and every number takes `100cqi / (chars x 0.82)` capped at 5.5rem, so a figure such as 43,605 or +27.8% can never outgrow its cell (fixed 77px numbers had collided in 250px cells). With a narrative the band splits at lg into one column of full-size figures beside the story, the stats column stretching to the story's height with its rows sharing it equally; two by two from sm to lg; one per row below sm. No vertical dividers: each stat keeps the shared top rule that draws with its count. `counter.js` counts comma-grouped values as one number. **Metric and stat labels, and card summaries, never carry dates** ("in August 2026", "to 27 August"): the case study already shows its year, and that level of detail is not wanted. Periods and measurement basis stay in the Results prose only. When checking any stat or figure, measure the ink against its own cell (a Range's rect against the cell's content box) at every width; a page-level overflow check cannot see it.

## Enquiry form system (July 2026)

The multi-step enquiry Gravity Form (id 5, "Enquiry Form - Contact", cssClass `enquiry-form`): four steps (Services picker, Project, About You, Contact), conditional logic driven by the service picker, per-step server validation, honeypot plus the inherited reCAPTCHA v3, dataLayer step/submit events. Live on the Contact page only; form 2 still serves the homepage and services-hub embeds and is the landing template's default CTA form. A single-step sibling exists too (id 6, "Enquiry Form - Simple", cssClass `enquiry-form enquiry-form--simple`: name pair, email, optional phone, service select) which inherits all the styling by class composition, needs no JS bundle, and is not embedded anywhere yet; a second single-step clone (id 7, "Enquiry Form - Free Website", form 6 minus the service select, cssClass `enquiry-form enquiry-form--simple enquiry-form--free-website`) serves the `/free-website-for-small-business/` offer page via `vc_render_form( 7 )`. All theme code keys on the `enquiry-form` cssClass, never the form id (the id appears only in the contact template's shortcode). Styling is the shared `common/_enquiry-form.scss`, scoped under `.gform_wrapper.enquiry-form_wrapper` so it travels with the form; behaviour is the `js/enquiry-form.js` bundle (entrance animation on `gform_post_render` + double rAF, never `gform_page_loaded`); the theme's first GF PHP hooks live in `inc/gravity-forms.php`. Verified at 375/768/1024/1440 in both modes. Full build notes, the verified GF mechanics, the gotchas (checkbox `inputs` array, Orbital specificity fights) and how to move the form to another page are in `docs/enquiry-form-system.md`: read that before touching the form, the partial, the bundle or the hooks file.

## Free website offer page (July 2026)

The standalone conversion landing page at `/free-website-for-small-business/` (page 880, `page-templates/page-free-website.php`): a free website build for local businesses, paid for by a rolling monthly fee from £49. Its own look within the forge language: slim header (logo + toggle + one CTA, no nav; the CTA hides below sm), typographic hero with the outlined £0 monument, three-step rail, the itemised ledger with the red £0 total, a paired contrast grid, the shared testimonial spotlight, a new FAQ accordion (server-rendered open, collapsed by JS) and Gravity Form 7 on a closing dark band, plus a mobile sticky CTA bar. FAQPage + Service JSON-LD; indexable; not in any menu. All copy in `fw_` ACF fields. Verified at 375/768/1024/1440 in both modes. Full build notes in `docs/free-website-page.md`: read that before touching the template, the `fw_` fields, the `free-website` bundle or the header/footer conditionals.
...
## Trust signals (Aug 2026)

Partner badges, the Google rating and the press credit: all edited on Global Settings, rendered by shared parts, built Aug 2026 (feat/trust-signals).

- **Google reviews source:** `inc/google-reviews.php` registers the Reviews group on Global Settings (`google_rating`, `google_review_count`, `google_rating_date` date picker returning `M Y`, `google_profile_url`) and defines `vc_google_reviews()` returning `['rating','count','url','as_of']` through the `vc_google_reviews` filter, the drop-in point for a future Places API fetch (no template changes needed). Every consumer reads the helper only.
- **Rating chip:** one shared partial `template-parts/rating-chip.php` replaced the three pasted copies (homepage testimonials head, About proof, services hub proof). With a count and URL saved it renders as an `<a>` to the profile: stars row kept away from the G per Google's brand rules, the full-colour G (`assets/images/logos/google-g.webp`, never filtered or recoloured, `alt=""`) on the label line, the compact wording **"5.0 · N Google Reviews"** (Ibrar's wording, Aug 2026; Google's own guidance prefers "reviews on Google", never "Google rating", so do not drift further from it), and a quiet as-of line on the chip only (Google's guidelines ask for one beside an overall figure; the footer credit carries no date by Ibrar's choice). Without a count or URL it falls back to the neutral unlinked "average client rating" presentation. Hover/focus is border heat + label underline only (styles extend `.rating-chip` in `common/_testimonial-spotlight.scss`). The old `hp_testimonials_rating_value/label` fields were retired: removed from homepage.json, front-page meta deleted, and the About/hub cross-page reads dropped.
- **Review tokens:** `vc_review_tokens( $text )` (same file) swaps `{rating}` and `{reviews}` in editor-entered text for the live Global Settings figures; the homepage `hp_results_stats` value/label run through it, and the saved second stat row is `{rating}` / "Rating on Google", so the rating is typed once. Use the tokens anywhere else a stat repeats the figure.
- **No review structured data, on purpose (standing rule).** Google ignores self-serving AggregateRating/Review markup on an organisation's own pages and bars marking up ratings sourced from Google itself, so the theme outputs none anywhere; the visible profile link is the verification route. Do not "fix" this by adding review schema.
- **Partner badge strip:** `template-parts/partner-logos.php` + `common/_partner-logos.scss`, fed by the Partner Logos group (`inc/trust-signals-fields.php`: `partner_logos_heading` (homepage strip label), `partner_logos_footer_label` (footer label, default "Official partners") + `partner_logos` repeater: `logo`, `name`, `url`). Official colour badges as issued, never filtered or knocked out; area-based sizing (`is-wide` at ratio >= 1.6: squares 64px, wide lockups 30px; 48/24 below md; the footer variant 40/20). Two live hosts, both dark in every mode (Ibrar's review, Aug 2026: the About proof instance was removed and the badges moved into the footer so they show sitewide): the homepage why band close (`surface => 'dark'`, a static centred row on a hairline seam, revealed via the homepage IO fade list) and the footer trust row (`variant => 'footer'`, class `.is-footer`: no seam of its own, label in the `.footer-heading` voice, smaller left-aligned badges). The `auto` surface (light sections with the dark-mode pairing) is kept for future placements; the earlier per-row light-surface file and "dark only" toggle were retired with it, so the supplied white Shopify and Hotjar artwork needs no variants. No Splide, no marquee, no new motion. Empty repeater renders nothing. Explicit width/height attributes are the CLS defence, preferred over the hero marquee's reserved-height trick.
- **Footer trust row:** a `col-lg-12` between the contact column and `.bottom` in `footer.php`, on the same 40px hairline seam recipe, holding three groups bottom-aligned on one row at lg+ and stacked below: `.footer-press` ("As featured in" as a `.footer-heading` label over the Press Features wordmarks, white knockout, 20px, linked; the row has a 40px floor so the labels share a line with the badges'), `.footer-partners` (the partner strip's footer variant, label over the badge row) and `.footer-google` (the G + "5.0 · N Google Reviews" linking to the profile, no date). Each group renders only with its own data; all three empty removes the row. Shows on every template, slim-header landing pages included. Press logos come from the `press_features` repeater (`inc/trust-signals-fields.php`; `logo`, `name`, `url`), whose first row's logo also marks the About press credit line.
- **About press section (the print run, Ibrar's second review, Aug 2026):** `.about-press` after `.about-proof` (#F5F5F5 light / #121212 dark, continuing the alternation), `ab_press_*` fields in about-us.json: a split head (three-part heading left; `ab_press_body` + ghost CTA `ab_press_link_label`/`ab_press_link_url` right, new tab; no wordmark or credit line, both rejected), then the **print run** across the band's base: `ab_press_image` (the feature spread, seeded as a 1708px WebP) repeated as overlapping, fanned copies (`.press-print`, each copy overlapping the one before by `width - step`, all on one baseline, so every visible slice is the headline and the statue) on three slow CSS-keyframe rows (`.press-rack-row.is-front` 470/300px copies showing the left page; `.is-back` smaller, 35% opacity, running the other way and **stacked in reverse** via `z-index: calc(2 * set-count - i)` from each copy's printed `--i`, so its visible slice is the right-hand founders' page; `.is-far` smaller still at 10%), tilted as one `.press-rack-plane` (`rotate(-5deg)`, origin bottom-right so the right end sits on the seam) inside a fixed-height `.press-rack` stage (560/380px) that **starts behind the head** (`margin-top` -120/-64px, `pointer-events: none`, `z-index` under the `.press-head`) so the faint row reaches up under the copy and no row is ever cut flat by the stage's top; the stage's `mask-image` sinks its top into the band and clips the copies at the seam; hairline paper edges, 2px corners, `grayscale(0.25)` rest, no shadows. **Seamless loop rule:** the template prints each row's copy count as `--set-count` (14/19/26, sized so one set outspans the plane up to 2560px wide), the SCSS derives `--set-w: count x step`, sizes the track at exactly two sets and the keyframe slides by exactly one set (never `-50%` of a `max-content` track: negative overlap margins make that half inexact and the loop jumps). The band uses `overflow: clip`, never `hidden` (hidden makes it a scroll container that anchors and focus scroll internally by the run's overflow), and `padding-bottom: 0` at every width (the section mixin's small-screen padding must be re-zeroed). The rack is `aria-hidden` (decorative; the head carries the information, copies carry `alt=""`). Reveals: `.press-body`, `.press-cta` and `.press-rack` fade (the tilt lives on the inner plane, never on the fade target); the marquee is a plain CSS keyframe loop (the homepage text marquee it was modelled on was removed in Sep 2026) and the reduced-motion safety net (plus an explicit `animation: none`) leaves a still rack. The section hides when every raw press field is blank; no image hides the run. The template tolerates a bare attachment ID from the image field (`acf_get_attachment()` fallback), after a programmatic group import once rewrote the field as `text` and fatalled the page.
- **Deploy replay (live DB side, run at deploy like the Content & Social pack):** the code carries no data, so on production: (1) seed the four Reviews options (`update_field` with keys `field_vc_google_rating` 5.0, `field_vc_google_review_count`, `field_vc_google_rating_date` Ymd, `field_vc_google_profile_url` = the maps.app.goo.gl share link); (2) upload the five trimmed partner badges + the Your Business wordmark (`assets/images/press/your-business-wordmark.png` is the repo seed source) + the feature spread WebP to the media library and seed `partner_logos_heading`, `partner_logos_footer_label`, the 5 `partner_logos` rows (sub-field keys: logo, name, url) and the `press_features` row (wordmark, name, the nxtbook URL); (3) seed the About page's `ab_press_*` values to mirror the JSON defaults plus `ab_press_image` (the spread attachment); (4) delete the orphaned `hp_testimonials_rating_value/label` meta (+ `_`-prefixed refs) on the front page; (5) sync the Homepage and About Us groups by passing the EXISTING `acf-field-group` post ID found by post_name into the import (never ID 0: it forks a duplicate group post) and call `kses_remove_filters()` first: a CLI run has no user, so WordPress KSES filters the serialised field definitions on save and any `>` in a field's text becomes `&gt;`, which breaks the serialisation and ACF falls back to a bare `text` field (this is what rewrote the image field in round 1 and the stats repeater in the clean-up; keep angle brackets out of field instructions too), then **verify every imported field's stored `type`** against the JSON; (6) if duplicate group posts ever need removing, detach `ACF_Local_JSON::delete_field_group` from `acf/trash_field_group` and `acf/delete_field_group` first and assert the JSON files still exist afterwards: the hook deletes the live JSON file for the shared key (it wiped homepage.json and about-us.json locally in Aug 2026; both were restored from git); (7) end with a fresh-process re-read verification and a LiteSpeed purge. The local DB now holds exactly one group post per key (#966 Homepage, #1030 About Us).

---

## Real work entries and the board imagery (Sep 2026)

The `[SAMPLE]` projects, case studies and testimonials were deleted on 16 Sep 2026 (`feat/real-work-entries`) and 13 real work entries published through the pipeline from `content/work/` in the content repo, with three testimonials taken verbatim from the Google reviews (`content/testimonials/`). Facts recorded with them:

- **Placement:** `date:` front matter (YYYY-MM-DD) sets `post_date`, which orders /work/, /case-studies/ and the service-page strips; the endpoint applies it on every run (`core.date`, see docs/publishing-pipeline.md). Ibrar's order, oldest first: Vulkan v1 (2024), Drake & Case, Kore Systems (2025), then Leigh Blanks, Bite Talent, Minds Behind the Bite, Now Sold, The Broker House, Source Pay, The Boundary SW6, Insight Systems, Arundel Fractional Finance, Vulkan v2 (2026).
- **Main images are composed boards, not device photos or raw screenshots** (Ibrar's direction, Sep 2026, from a Restore-style reference): a 2400 x 1500 `#121212` ground with the soft red radial glow, the client logo as a white knockout top right, and a left-weighted 1440 x 920 frame (10px corners, hairline border, edges kept, no fade) holding either the homepage captured at 1440px (websites) or a 3 x 2 grid of the real posts (social, paid, podcast). Everything sits inside the intersection of the centred 4:3 crop (x 200 to 2200) and the centred 21:9 crop (y 236 to 1264), because one `pj_image` feeds the 4:3 tiles, the 16:10 cards and the 21:9 hero. Square marks get a 220px box or a monogram renders as a speck at tile size. The generator (`make-board.py` + `board.css`, rendered headless at 2400 x 1500) lives with the session; rebuild it from this recipe rather than compositing by hand, and always dismiss cookie banners before a capture. Gallery images are flat 1440 x 900 captures of inner pages (captions sit below them, so no overlay concerns).
- **Placeholders:** Leigh Blanks, Minds Behind the Bite, Now Sold and The Broker House carry an empty frame board until Ibrar supplies the artwork; replace the board, re-run the file with `--overwrite`.
- **Personal Branding** is a child service under Branding (term 55, `/services/branding/personal-branding/`), created for the individual-client work (Leigh Blanks, Sonny Dickinson at Bite Talent); its `sv_` copy was seeded on creation and it appears in the mega menu automatically.
- **Homepage empty states:** with no featured case study the `.work` section is absent and `.our-work` takes the white pairing (`is-first-work`); with nothing curated the wheel section is absent too. The Our Work wheel is curated to Insight, Arundel, Kore, Drake & Case, Vulkan v2, Bite Talent, Source Pay and The Boundary.
- **Live prerequisites at deploy:** none of this is data in the theme. On the live database the same files publish through the pipeline once the parked live checklist has run; the Personal Branding term and its copy must be created there first (the pipeline never invents taxonomy).
...
Client side: the `vc-publish` Node CLI and all content sources live in `~/repos/vulkancreative-content` (a separate repo, so client drafts and source images never ride a theme deploy). Authoring runs through five global skills: `vulkan-creative-publish` (mechanics), plus the interview-driven `vulkan-creative-blog`, `vulkan-creative-work`, `vulkan-creative-case-study` and `vulkan-creative-landing-page`. All five are invocation-only (`disable-model-invocation`): they fire on an explicit `/name` and never on trigger phrases, Ibrar's standing rule for client-specific skills. Full endpoint contract, per-type format reference and the PARKED live-rollout checklist: `docs/publishing-pipeline.md`. The pipeline targets the local site only until that checklist is run deliberately.
...
- **`inc/`** -- Modular PHP includes:
  ...
  - **`actions.php`** -- `vc_setup()` registers theme supports (title-tag, post-thumbnails, responsive-embeds, html5, woocommerce), a custom image size `header-image` (1920x1080), content width 1920, and nav menus. `vc_security_headers()` sends nosniff, SAMEORIGIN, a referrer policy and a Permissions-Policy (HSTS stays with the host). The old `vc_enqueue()` (style.css plus a forced jQuery) and `vc_footer()` (a user-agent sniff) were removed in Sep 2026. Also handles favicon output (custom SVG/PNG suite), disables the content editor on the default `post` type, and registers footer menus. Image sizes: `header-image` (1920x1080) and `vc-1440`.

  - **`filters.php`** -- Sets the document title separator to `|`. Sanitises empty post titles to `'...'`. Adds `itemprop="url"` to nav menu links for schema.org. Disables the two largest core image sizes (`1536x1536`, `2048x2048`); `medium_large` (768w) is back on since Sep 2026 so a srcset no longer jumps from 300w to 1024w. Overrides the post link for `partner_content` CPT to use a `link_to_content` meta field. Adds body classes: `'dark-mode'` always (dark-first default; an inline script in `header.php` removes it for light-mode visitors, see Dark Mode), and `'practice-area-parent'`/`'practice-area-child'` for the `practice_area` taxonomy. Removes the last (duplicate) breadcrumb item from Yoast breadcrumbs. A `wpseo_schema_organization` filter enriches Yoast's Organization node (`@id` `/#organization`) with the `company_*` contact details from Global Settings and the four social profile URLs, so the knowledge graph matches the visible site (the About page's Person nodes attach to the same `@id`).
...
- The header logo is inlined via `file_get_contents()` and draws in with CSS keyframes (no GSAP in the header bundle); the footer logo is an `<img>` of `logo-white.svg`. `Archivo-Display.woff2` and three Latin-subset Poppins weights are preloaded.
- Footer carries a CTA row (hidden on Contact), a `.footer-contact` column reading the same Global Settings `company_*` fields as the Contact page, the Explore menu, socials, and an oversized 5%-opacity "Vulkan" wordmark (printed from CSS `::before`, rising on a CSS scroll-driven animation where supported) clipped by `overflow: hidden`.
...
- **Per-page picker** (`vc_page_form`, group `group_vc_form_settings`): a select in the "Form Settings" side panel on the front page and the Contact / Services Hub / Free Website / Landing templates. One saved value per page. Defaults: homepage/services-hub `2`, contact `5` (the multi-step enquiry form, id 5); the landing template’s CTA block defaults to `10` ("Enquiry Form - Landing") and each landing page picks its own (e.g. `/seo-ai-search/` uses form 8).
...
**`service`** (registered in `custom-taxonomies.php`): hierarchical, on `post`, `project` and `case_study` types. Rewrite slug `'services'` (changed from `'service'` in July 2026; old URLs and retired pillar slugs 301 via the shim in `inc/actions.php`, driven by `vc_service_slug_aliases()`; flush rewrite rules after any change). ACF field: `icon` (text). Pillar ordering, per-location visibility and (since Aug 2026) each pillar's child order live in the global **Service list** on Global Settings (`vc_ordered_services()` / `vc_service_children()`), replacing the old per-term `order` field (July 2026). Feeds the homepage `.service-rail`, each term's own page (`taxonomy-service.php`), and a card on the services hub. Field retrieval: `get_field('icon', 'service_' . $term->term_id)`. Every term page is indexable (Aug 2026).
...
- **Buttons are white on the button red.** Since Sep 2026 `vc-button-forge`, the Gravity Forms buttons, the pagination current state and the play button use `$vc-button-red` (#D93B30, 4.55:1 with white; hover `$vc-button-red-hover` #DB3B00, 4.53:1), Ibrar's choice so labels pass 4.5:1. Every other red stays `$vc-primary` #FF3B30; do not swap the two.
...
- **Global/options fields:** no prefix (e.g. `worked_with_logos`, `company_email`/`company_phone`/`company_location`/`company_map_url`, and since Aug 2026 the trust-signal fields `google_rating`/`google_review_count`/`google_rating_date`/`google_profile_url`, `partner_logos_heading` + `partner_logos`, `press_features`), referenced with the `'options'` post id.
...
|---|---|---|
| Bootstrap | 5.3.x | A subset only (reboot, containers, grid, a trimmed utilities map): see `app.scss` before reaching for a Bootstrap class |
| GSAP | 3.13 | Animations (ScrollTrigger, SplitText). Not in the header or footer |
...
| jQuery | WP core | Not a theme dependency since Sep 2026. Gravity Forms loads it, in the footer, on pages that render a form; `enquiry-form/steps.js` uses it there |
...
3. Add a conditional `vc_enqueue_bundle()` call in `inc/styles-scripts.php`:
   ```php
   if ( is_page_template( 'page-templates/page-your-page-name.php' ) ) {
       vc_enqueue_bundle( 'your-page-name' );
   }
   ```
...
- **Image sizes:** Custom `header-image` (1920x1080) and `vc-1440`. `1536x1536` and `2048x2048` are disabled; `medium_large` is on. Print images through `vc_image()` (`inc/template-functions.php`) so they carry width, height and a srcset.
- **SVG logos:** The header logo is inlined via `file_get_contents()`; the footer uses `logo-white.svg` as an `<img>`.
...
- **Fonts:** Self-hosted woff2 in `assets/fonts/`: Poppins subset to Latin (`*-latin.woff2`) with the full files behind an extended `unicode-range`, and Archivo instanced to the display range in use (`Archivo-Display.woff2`). Four preloads in `header.php`.
