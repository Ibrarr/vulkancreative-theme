...
- **About press section:** `.about-press` after `.about-proof` (#F5F5F5 light / #121212 dark, continuing the alternation), `ab_press_*` fields in about-us.json: three-part heading, `ab_press_body`, a `.press-credit` line under the body (the first Press Features wordmark, 22px, greyscale light / knockout dark, beside `ab_press_credit`, default "Spring 2026 issue, pages 50 and 51"), ghost CTA `ab_press_link_label`/`ab_press_link_url` (new tab), and the **tear-sheet**: `ab_press_image` (the feature spread, seeded as a 1708px WebP) rendered in `.press-sheet` as a printed page lying across the band, `rotate(-4deg)` at lg+ (-3deg below), oversized to 124%/112% of its `.press-media` stage and hung off the stage bottom so the section's `overflow: hidden` cuts it at the bottom and the right edge; hairline paper edge, 2px corners (a page, not a media panel), `grayscale(0.25)` rest, no shadow. The fade reveal sits on `.press-media` (never on the rotated sheet: an inline GSAP transform would overwrite the tilt); `.press-body`/`.press-credit`/`.press-media` are in the About fades list. The section hides when every raw press field is blank; no image hides the stage and the credit line keeps the wordmark. The template tolerates a bare attachment ID from the image field (`acf_get_attachment()` fallback), after a programmatic group import once rewrote the field as `text` and fatalled the page.
- **Deploy replay (live DB side, run at deploy like the Content & Social pack):** the code carries no data, so on production: (1) seed the four Reviews options (`update_field` with keys `field_vc_google_rating` 5.0, `field_vc_google_review_count`, `field_vc_google_rating_date` Ymd, `field_vc_google_profile_url` = the maps.app.goo.gl share link); (2) upload the five trimmed partner badges + the Your Business wordmark (`assets/images/press/your-business-wordmark.png` is the repo seed source) + the feature spread WebP to the media library and seed `partner_logos_heading`, `partner_logos_footer_label`, the 5 `partner_logos` rows (sub-field keys: logo, name, url) and the `press_features` row (wordmark, name, the nxtbook URL); (3) seed the About page's `ab_press_*` values to mirror the JSON defaults plus `ab_press_image` (the spread attachment) and `ab_press_credit`; (4) delete the orphaned `hp_testimonials_rating_value/label` meta (+ `_`-prefixed refs) on the front page; (5) sync the Homepage and About Us groups by passing the EXISTING `acf-field-group` post ID found by post_name into the import (never ID 0: it forks a duplicate group post; the local DB already carries pre-existing duplicates from old syncs), then **verify every imported field's stored `type`** (the round-1 import silently rewrote the image field as `text`); (6) end with a fresh-process re-read verification and a LiteSpeed purge.
