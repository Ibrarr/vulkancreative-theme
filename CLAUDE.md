...
- **Partner badge strip:** `template-parts/partner-logos.php` + `common/_partner-logos.scss`, fed by the Partner Logos group (`inc/trust-signals-fields.php`: `partner_logos_heading` label + `partner_logos` repeater: `logo`, optional `logo_light`, `name`, `url`, `dark_only` toggle). Official colour badges as issued, never filtered or knocked out; area-based sizing (`is-wide` at ratio >= 1.6: squares 64px, wide lockups 30px; 48/24 below md); a static centred row (no Splide, no marquee), revealed only via the pages' existing IO fade lists. Args: `surface` `'dark'` (the homepage why band close) or `'auto'` (About proof). On auto a `dark_only` row without a `logo_light` hides in light mode (the supplied Shopify and Hotjar files are white artwork; drop light-legible files into `logo_light` and untick to show all five everywhere). Empty repeater renders nothing. Explicit width/height attributes are the CLS defence, preferred over the hero marquee's reserved-height trick.
- **Footer trust row:** a `col-lg-12` between the contact column and `.bottom` in `footer.php`, on the same 40px hairline seam recipe: "As featured in" + the Press Features logos (white knockout) left, the G + "5.0 from N reviews on Google · as-of" linking to the profile right. Either side renders alone; both empty removes the whole row. Shows on every template, slim-header landing pages included. Press logos come from the `press_features` repeater (`inc/trust-signals-fields.php`; `logo`, `name`, `url`), whose first row's logo doubles as the About press media fallback.
- **About press section:** `.about-press` after `.about-proof` (#F5F5F5 light / #121212 dark, continuing the alternation), `ab_press_*` fields in about-us.json: three-part heading, `ab_press_body`, optional `ab_press_image` cover (10px media panel, resting desaturation) with the Press Features wordmark as fallback (greyscale light / knockout dark), ghost CTA `ab_press_link_label`/`ab_press_link_url` (new tab). The section hides when every raw press field is blank; the fallbacks only cover partial blanks. The h2 and body are registered in `about/reveal.js`.
- **Deploy replay (live DB side, run at deploy like the Content & Social pack):** the code carries no data, so on production: (1) seed the four Reviews options (`update_field` with keys `field_vc_google_rating` 5.0, `field_vc_google_review_count`, `field_vc_google_rating_date` Ymd, `field_vc_google_profile_url` = the maps.app.goo.gl share link); (2) upload the five trimmed partner badges + the Your Business wordmark (`assets/images/press/your-business-wordmark.png` is the repo seed source) to the media library and seed `partner_logos_heading`, the 5 `partner_logos` rows (sub-field keys; `dark_only` = 1 on Shopify and Hotjar) and the `press_features` row (wordmark, name, the nxtbook URL); (3) seed the About page's `ab_press_*` values to mirror the JSON defaults; (4) delete the orphaned `hp_testimonials_rating_value/label` meta (+ `_`-prefixed refs) on the front page; (5) sync the Homepage and About Us groups by updating the EXISTING `acf-field-group` post found by post_name (never import with ID 0: it forks a duplicate group post; the local DB already carries pre-existing duplicates from old syncs, see the audit note in the session report); (6) end with a fresh-process re-read verification and a LiteSpeed purge.
