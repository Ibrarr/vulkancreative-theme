...
- **Review round (17 Sep 2026):** Splide lists and slides are `<div>`s (Splide's `role="group"` is invalid on `<li>`), which takes Lighthouse's Agentic Browsing to 3/3; process steps fill their own top rule at lg and up (`.step-fill`), never the gap between steps; `vc_service_icon_url()` stops children borrowing their pillar's icon, and all 20 children now carry their own Slab Regular mark (Pro SVGs come from the account's **kit** CDN, not the API token, which only holds the free scope; the recipe and the house 576-square normalisation are in `docs/prelaunch-audit.md`); author photos take the About duotone; the About film is a self-hosted 1080p encode.
